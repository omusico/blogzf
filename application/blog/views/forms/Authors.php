...
class forms_Users extends Zend_Form
{
    public function __construct ($user = null, $options = null)
    {
        ...
        $username->setLabel('Email')->addFilter('StringToLower')->setRequired(true)->addValidator('NotEmpty', true)->addValidator('EmailAddress')->setValue($usernameValue);
        $password = new Zend_Form_Element_Password('password');
        $password->setLabel('Clave')->setRequired(true)->addValidator('NotEmpty');
        $displayName = new Zend_Form_Element_Text('display_name');
        $displayName->setLabel('Nombre que aparecera en la pagina')->setRequired(true)->addValidator('NotEmpty')->setValue($displayNameValue);
        $status = new Zend_Form_Element_Text('status');
        $status->setLabel('Estado')->setValue($statusValue)->setAttrib('readOnly', true);
        $submit = new Zend_Form_Element_Submit('submit');
        $submit->setLabel('Enviar');
        $this->addElements(array($username , $password , $displayName , $status , $submit));
    }
}